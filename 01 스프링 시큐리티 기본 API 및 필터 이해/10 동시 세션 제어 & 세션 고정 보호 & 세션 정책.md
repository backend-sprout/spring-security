...
공격자는 서버에 접속을 하면서 JSESSIONID를 발급 받는다.              
공격자는 이후 발급받은 JSESSIONID를 일반 사용자에게 몰래 넘긴다.          
일반 사용자는 공격자의 JSESSIONID를 통해 로그인이 진행이 되고 성공할 것이다.           
일반 사용자와 공격자는 같은 쿠키로 인증이 되었으므로 정보를 공유할 수 있고      
공격자에서는 공유된 일반 사용자의 정보를 탈취한다.       
